...
if (isset($_POST["userID"])) {
	...
	if ($accessResult == 0) {
		...
		if (is_numeric($sUserID)) {
			...
			$result = mysql_query("select userTag.tagID,tagInfo.tagName FROM userTag INNER JOIN tagInfo ON userTag.tagID = tagInfo.tagID WHERE userID = $sUserID ORDER BY tagPoint desc");
